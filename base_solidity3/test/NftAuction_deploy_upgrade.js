...
describe("Starting", async function () {
    it("Should be able to deploy", async function () {
        ...
        const auctionFactoryV2 = await ethers.getContractAt("NftAuctionFactoryV2", auctionFactoryProxyAddr);
        expect(typeof auctionFactoryV2.factoryV2 === 'function').to.be.true;
        expect(await auctionFactoryV2.factoryV2()).to.equal("factoryV2");

        const auctionV2Addr = await auctionFactoryV2.getAuction(tokenId);
        const auctionV2 = await ethers.getContractAt("NftAuctionV2", auctionV2Addr);
        expect(typeof auctionV2.upgradeTest === 'function').to.be.true;
        expect(await auctionV2.upgradeTest()).to.eq("upgradeTest");
        expect(await auctionV2.nftContractAddress()).to.eq(myNFTAddr);
    });
